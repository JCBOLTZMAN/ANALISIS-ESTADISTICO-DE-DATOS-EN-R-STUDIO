...
#   - \fancyhf{}
#   - \rhead{\thepage}
#   - \lhead{ANÁLISIS ESTADÍSTICO DE DATOS EN R-STUDIO}
#   - \lfoot{Mas Ciencia}
#   - \rfoot{\includegraphics[width=0.4in]{img/5.png}}
#   - \cfoot{}
...
rango = qr(m)$rank
```
Para la siguiente matriz el rango[^2] es: $r_m = `r rango`$ 
```{r results='markup'}
kable(m)
```

[^2]: [Rango de una matriz](https://ocw.ehu.eus/pluginfile.php/40253/mod_resource/content/1/fundamen_mate/contenidos/ejercicios/ejercicios-resueltos/rango-de-una-matriz.pdf)
...
Para la siguiente tabla la traza[^3] es igual a: $T_r(A) = `r tr`$ 

`r kable(m)`

[^3]: [Traza de una matriz](https://www.universoformulas.com/matematicas/algebra/traza-matriz/)
...
m=cbind(C1,C2,C3,C4,C5)
corr_m = round(cor(m),4)
```
`r kable(headTail(corr_m))`
...
coef(solucion)
```
Donde $x = `r coef(solucion)[1]`$, $y = `r coef(solucion)[2]`$
